...
const int PkgAscii::CHARACTER_PROPERTIES[128] =
{
	/* 00 . */ ACP_CONTROL,
	/* 01 . */ ACP_CONTROL,
	/* 02 . */ ACP_CONTROL,
...
//----------------------------------------------------------------------------
int PkgAscii::Properties(int ch)
{
	if (IsAscii(ch))
...
//----------------------------------------------------------------------------
bool PkgAscii::IsAscii(int ch)
{
	return (static_cast<int>(ch)& 0xFFFFFF80) == 0;
}
//----------------------------------------------------------------------------
bool PkgAscii::HasProperties(int ch, int props)
{
	return (Properties(ch) & props) == props;
}
//----------------------------------------------------------------------------
bool PkgAscii::HasSomeProperties(int ch, int props)
{
	return (Properties(ch) & props) != 0;
}
//----------------------------------------------------------------------------
bool PkgAscii::IsSpace(int ch)
{
	return HasProperties(ch, ACP_SPACE);
}
//----------------------------------------------------------------------------
bool PkgAscii::IsDigit(int ch)
{
	return HasProperties(ch, ACP_DIGIT);
}
//----------------------------------------------------------------------------
bool PkgAscii::IsHexDigit(int ch)
{
	return HasProperties(ch, ACP_HEXDIGIT);
}
//----------------------------------------------------------------------------
bool PkgAscii::IsPunct(int ch)
{
	return HasProperties(ch, ACP_PUNCT);
}
//----------------------------------------------------------------------------
bool PkgAscii::IsAlpha(int ch)
{
	return HasProperties(ch, ACP_ALPHA);
}
//----------------------------------------------------------------------------
bool PkgAscii::IsAlphaNumeric(int ch)
{
	return HasSomeProperties(ch, ACP_ALPHA | ACP_DIGIT);
}
//----------------------------------------------------------------------------
bool PkgAscii::IsLower(int ch)
{
	return HasProperties(ch, ACP_LOWER);
}
//----------------------------------------------------------------------------
bool PkgAscii::IsUpper(int ch)
{
	return HasProperties(ch, ACP_UPPER);
}
//----------------------------------------------------------------------------
int PkgAscii::ToLower(int ch)
{
	if (IsUpper(ch))
		return ch + 32;
	else
