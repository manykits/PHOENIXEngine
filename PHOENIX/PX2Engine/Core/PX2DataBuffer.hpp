...
#ifndef PX2DATABUFFER_HPP
#define PX2DATABUFFER_HPP
...
namespace PX2
{
	...
	public:
		DataBuffer(std::size_t capacity) :
			...
			_ptr(0),
			_ownMem(true)
			/// Creates and allocates the DataBuffer.
		{
			if (capacity > 0)
			{
				_ptr = new T[capacity];
		...
		DataBuffer(T* pMem, std::size_t length) :
			_capacity(length),
			_used(length),
			_ptr(pMem),
			_ownMem(false)
			/// Creates the DataBuffer. Length argument specifies the length
			/// of the supplied memory pointed to by pMem in the number
			/// of elements of type T. Supplied pointer is considered
			/// blank and not owned by DataBuffer, so in this case DataBuffer 
			/// only acts as a wrapper around externally supplied 
			/// (and lifetime-managed) memory.
		{
		}

		DataBuffer(const T* pMem, std::size_t length) :
			_capacity(length),
			_used(length),
			_ptr(0),
			_ownMem(true)
			/// Creates and allocates the DataBuffer; copies the contents of
