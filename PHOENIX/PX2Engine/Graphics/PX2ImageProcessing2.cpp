...
#include "PX2ImageProcessing2.hpp"
...
mBound1(bound1)
{
	...
	// 创建处理的材质
	CreateMaterial(mainPShader, material, instance);
	mMainMaterial = material;
	mMainMaterialInstance = instance;
	mMainMaterialInstance->SetPixelTexture(0, "StateSampler",
		mTargets[1]->GetColorTexture(0));

	// Create the boundary material.
	if (useDirichlet)
	{
		CreateBoundaryDirichletMaterial(material, instance);
	}
	else
	{
		CreateBoundaryNeumannMaterial(material, instance);
	}
	mBoundaryMaterial = material;
	mBoundaryMaterialInstance = instance;
	mBoundaryMaterialInstance->SetPixelTexture(0, "StateSampler",
		mTargets[0]->GetColorTexture(0));

	// 创建绘制最终处理结果的材质
	CreateDrawMaterial(material, instance);
	mDrawMaterial = material;
	mDrawMaterialInstance = instance;
	mDrawMaterialInstance->SetPixelTexture(0, "StateSampler",
		mTargets[1]->GetColorTexture(0));
}
//----------------------------------------------------------------------------
