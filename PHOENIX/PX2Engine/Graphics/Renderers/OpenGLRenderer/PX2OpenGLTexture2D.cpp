...
mPreviousTexture(0)
{
	Texture::Format tf = texture->GetFormat();
	mIsDepthSencil = (tf == Texture::TF_D16 || tf == Texture::TF_D24S8);

	mInternalFormat = gOGLTextureInternalFormat[tf];
	mFormat = gOGLTextureFormat[tf];
	mType = gOGLTextureType[tf];
