...
PX2_IMPLEMENT_RTTI(PX2, Polysegment, CurveMesh);
PX2_IMPLEMENT_STREAM(CurveMesh);
PX2_IMPLEMENT_FACTORY(CurveMesh);
...
Object* CurveMesh::GetObjectByName (const std::string& name)
{
	...
	PX2_GET_OBJECT_BY_NAME(mOrigVBuffer, name, found);
	PX2_GET_OBJECT_BY_NAME(mOrigParams, name, found);
	if (mSegments)
	{
		for (int i = 0; i < mNumSegments; ++i)
		{
			PX2_GET_OBJECT_BY_NAME(mSegments[i], name, found);
		}
	}
									 ...
									 std::vector<Object*>& objects)
{
	...
	PX2_GET_ALL_OBJECTS_BY_NAME(mOrigVBuffer, name, objects);
	PX2_GET_ALL_OBJECTS_BY_NAME(mOrigParams, name, objects);
	if (mSegments)
	{
		for (int i = 0; i < mNumSegments; ++i)
		{
			PX2_GET_ALL_OBJECTS_BY_NAME(mSegments[i], name, objects);
		}
	}
}
//----------------------------------------------------------------------------
...
//----------------------------------------------------------------------------
int CurveMesh::GetStreamingSize (Stream &stream) const
{
	int size = Polysegment::GetStreamingSize(stream);
	size += PX2_VERSION_SIZE(mVersion);

	size += sizeof(mNumFullVertices);
	size += sizeof(mNumSegments);
	size += sizeof(mLevel);
	size += PX2_BOOLSIZE(mAllowDynamicChange);
	size += PX2_POINTERSIZE(mOrigVBuffer);
	size += PX2_POINTERSIZE(mOrigParams);

	if (mSegments)
	{
		size += mNumSegments*PX2_POINTERSIZE(mSegments[0]);
	}

	if (mCInfo)
	{
		size += mNumFullVertices*PX2_POINTERSIZE(mCInfo[0].Segment);
