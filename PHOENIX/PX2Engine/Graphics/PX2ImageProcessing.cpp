	...
	if (renderer->PreDraw())
	{
		...
		if (openglHack)
		{
			...
			mMainMaterialInstance->SetPixelTexture(0, "StateSampler",
				mMainTexture);
			mRectangle->SetMaterialInstance(mMainMaterialInstance);
		}
		else
		{
			mDrawMaterialInstance->SetPixelTexture(0, "StateSampler",
				mMainTexture);
			mRectangle->SetMaterialInstance(mDrawMaterialInstance);
		}
		...
		mRectangle->SetMaterialInstance(mBoundaryMaterialInstance);
...
void ImageProcessing::ExecuteStep (Renderer* renderer, bool draw)
{
	...
	// Take a step using the main material.
	mRectangle->SetMaterialInstance(mMainMaterialInstance);
	renderer->Enable(mTargets[0]);
	renderer->Draw(mRectangle);
	renderer->Disable(mTargets[0]);
...
void ImageProcessing::CreateVertexShader ()
{
	...
	mVertexShader = new0 VertexShader("PX2.ScreenShader",
		1, 2, 1, 0, false);
