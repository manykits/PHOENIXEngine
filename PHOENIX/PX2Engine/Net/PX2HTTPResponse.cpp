...
Timestamp HTTPResponse::GetDate() const
{
	...
	int tzd;
	return DateTimeParser::parse(dateTime, tzd).GetTimestamp();
}
//----------------------------------------------------------------------------
...
void HTTPResponse::read(std::istream& istr)
{
	...
	while (PX2::Ascii::IsSpace(ch)) ch = istr.get();
	while (!PX2::Ascii::IsSpace(ch) && ch != eof && status.length() < MAX_STATUS_LENGTH) { status += (char)ch; ch = istr.get(); }
	if (!PX2::Ascii::IsSpace(ch)) throw MessageException("Invalid HTTP status code");
	while (PX2::Ascii::IsSpace(ch) && ch != '\r' && ch != '\n' && ch != eof) ch = istr.get();
	while (ch != '\r' && ch != '\n' && ch != eof && reason.length() < MAX_REASON_LENGTH) { reason += (char)ch; ch = istr.get(); }
	if (!PX2::Ascii::IsSpace(ch)) throw MessageException("HTTP reason string too long");
	if (ch == '\r') ch = istr.get();
