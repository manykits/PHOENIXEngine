...
inline LuaObject& LuaObject::Assign(lua_State* state, const char* value, int len) {
...
template <typename ValueT>
LuaObject& LuaObject::Assign(lua_State*state, const ValueT& value) {
	if (L)
		lua_fastunref(L, ref);
	L =state;
	LPCD::Type<ValueT>::Push(L, value);
...
template <typename ValueT>
LuaObject& LuaObject::Assign(lua_State*state, const ValueT& value, int len) {
	if (L)
		lua_fastunref(L, ref);
	L =state;
	LPCD::Push(L, value, len);
